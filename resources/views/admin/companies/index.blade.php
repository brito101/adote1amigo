...
@section('content')
    <section class="dash_content_app">
        ...
        <header class="dash_content_app_header pb-1">
            <h2 class="icon-building-o">ONGs</h2>
            ...
            <div class="dash_content_app_header_actions">
                ...
                @can('Cadastrar Empresas')
                    @if ((Auth::user()->hasAnyRole('Anunciante') && $companies->count() == 0) || Auth::user()->hasAnyRole(['Administrador', 'Gerente']))
                        <a href="{{ route('admin.companies.create') }}" class="btn btn-orange icon-plus ml-1">Criar ONG</a>
                    @endif
                @endcan
            </div>
        </header>
                                ...
                                <td>
                                    @can('Editar Empresas')
                                        <a href="{{ route('admin.companies.edit', ['company' => $company->id]) }}"
                                            class="text-orange" title="Editar ONG">{{ $company->social_name }}</a>
                                    @else
                                        {{ $company->social_name }}
                            @endif
                            </td>
